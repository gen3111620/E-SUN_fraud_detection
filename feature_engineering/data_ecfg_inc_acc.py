...
data = data[["txkey","bacno", "cano", "datetime", "ecfg"]]
...
#             btw_1s = df_card["datetime"].between(datetime_from , datetime_to-pd.offsets.DateOffset(seconds=1))
            acc_inc_ecfg = df_card[btw].ecfg.sum()/df_card[btw].ecfg.count()
            return acc_inc_ecfg
        df_card["ecfg_inc_acc"] = df_card["datetime"].apply(accumulate)
...
data = data[["txkey", "ecfg_inc_acc"]]

print(data.shape)
data.to_csv("data_ecfg_inc_acc.csv", index=False)
