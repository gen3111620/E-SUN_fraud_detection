...
data = data[["txkey","bacno", "cano", "datetime", "conam"]]
...
for person in list(set(data.bacno)):
    ...
    for card in list(set(df_person.cano)):
        ...
        def ex1d(t):
            global df_card
            datetime_to = t
            datetime_from = t - pd.offsets.DateOffset(hours=24)
            ...
            counts = btw.sum()
            mean = df_card[btw_1s].conam.mean()
            med = df_card[btw_1s].conam.median()
            return [counts, mean, med]
        df_card[["counts_ex1d", "cost_mean_ex1d", "cost_med_ex1d"]] = pd.DataFrame(df_card["datetime"].apply(ex1d).values.tolist(), index=df_card.index)
        df_list.append(df_card)

data = pd.concat(df_list)
data.fillna(0, inplace=True)

data = data[["txkey", "counts_ex1d", "cost_mean_ex1d", "cost_med_ex1d"]]

print(data.shape)
data.to_csv("data_ex1d.csv", index=False)