...
data["hr_stocn"] = data["hr"] + "_" + data["stocn"].astype("str")

lbl = preprocessing.LabelEncoder()
lbl.fit(list(data["hr_stocn"].values))
data["hr_stocn"] = lbl.transform(list(data["hr_stocn"].values))

data = data[["txkey","bacno", "cano", "hr_stocn"]]

df_1 = data.groupby(["bacno", "cano", "hr_stocn"]).size().reset_index(name="counts")
df_2 = data.groupby(["bacno", "cano"]).size().reset_index(name="total_counts")

df_3 = df_1.merge(df_2)
df_3["hr_stocn_prop"] = df_3["counts"]/df_3["total_counts"]
df_3.drop(["counts", "total_counts"], axis=1, inplace=True)

data = data.merge(df_3, how="outer")

data[["txkey", "hr_stocn_prop"]].to_csv("hr_stocn_prop.csv", index=False)